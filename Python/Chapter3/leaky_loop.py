...
ax.plot(epsilon, gamma_resistive_approx, '--', color = clr, label = r'$2.2v_{A0} / (L_{ph0} \omega_1)$')
ylim0 = ax.get_ylim()
ax.plot(epsilon, gamma_leaky_approx, ':', color = clr, label = r'$2(\eta+\nu)\left(\frac{\omega L_z}{a_0v_{A0}|\ln R|}\right)^2/\omega_1$')
ax.set_xscale('log')
ax.set_ylim(ylim0)
...
ax.plot(f, 1 / tau_leakage(x0, R), label = r'$1 / \tau_{leakage}\ (\mathrm{s}^{-1})$')
ax.plot(f, 1 / tau_resistive(x0, omega), label = r'$1 / \tau_{resistive}\ (\mathrm{s}^{-1})$')
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles=handles, bbox_to_anchor=(0.5,1.2), loc='center')
ax.set_xscale('log')
ax.set_yscale('log')
...
ax.text(0.05, 0.675, \
	r'$v_{A0} = $' + '{:1.0E}'.format(vA0) + r' m$\,\mathrm{s}^{-1}$' + '\n' + \
	r'$a_0 = $' + '{:1.0E}'.format(a0) + ' m' + '\n' + \
	r'$\eta+\nu = $' + '{:1.0F}'.format(eta) + r' $\mathrm{m}^2\,\mathrm{s}^{-1}$' + '\n' + \
	r'$\omega=2\pi f$', \
	transform=ax.transAxes)

ax.text(0.5, 0.05, \
	r'$L_z = $' + '{:1.0E}'.format(Lz) + r' m' + '\n' +  \
	r'$h = $' + '{:1.1E}'.format(h) + r' m', \
	transform=ax.transAxes)
