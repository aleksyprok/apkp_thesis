...
ax.plot(x / abs(xi), ux[iz0,:].real * abs(xi), linewidth = 3, label = 'Numerical solution')
ax.plot(x / abs(xi), ux_ana(x,z0).real * abs(xi), label = 'Analytic solution')
ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
ax.set_title(r"Re[$u_x'(x,l_z) / u_0$]")
handles, labels = ax.get_legend_handles_labels()
lgd = ax.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5,1.5))

ax = fig.add_subplot(322)
ax.plot(x / abs(xi), ux[iz0,:].imag * abs(xi), linewidth = 3)
ax.plot(x / abs(xi), ux_ana(x,z0).imag * abs(xi))
ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
ax.set_title(r"Im[$u_x'(x,l_z) / u_0$]")
text1 = ax.text(-0.05,1.175, \
...
ax.plot(x / abs(xi), u_perp[iz0,:].real * abs(xi), linewidth = 3)
ax.plot(x / abs(xi), u_perp_ana(x,z0).real * abs(xi))
ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
ax.set_title(r"Re[$u_\perp'(x,l_z) / u_0$]")

ax = fig.add_subplot(324)
ax.plot(x / abs(xi), u_perp[iz0,:].imag * abs(xi), linewidth = 3)
ax.plot(x / abs(xi), u_perp_ana(x,z0).imag * abs(xi))
ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
ax.set_title(r"Im[$u_\perp'(x,l_z) / u_0$]")

ax = fig.add_subplot(325)
ax.plot(x / abs(xi), Sx[iz0,:].real / Sx0, linewidth = 3)
ax.plot(x / abs(xi), Sx_ana(x,z0).real / Sx0)
ax.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
ax.set_xlabel(r'$x\, /\, |x_{i,11}|$')
ax.set_title(r"$\qquad\langle S_x(x,l_z) \rangle\, \slash\, |\langle \Delta S_x(l_z) \rangle|$")
...
ax.loglog(omega_i_array / omega_r, ux_err)
ax.set_title(r"max$\,|u_{x,num}' - u_{x,ana}'|\,\slash\,|u_{x,num}'|}$")
ax.set_xlabel(r'$\omega_i / \omega_r$')

fig.savefig('temp_figures/oblique_field_spectral_method_along_x.pdf', bbox_inches = 'tight')
