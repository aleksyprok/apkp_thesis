...
def variance_exact(t):
	return E / 2 / omega_n ** 3 * (omega_n * t - np.sin(omega_n * t) * np.cos(omega_n * t))
...
a = np.full(N+1, xi ** 0.9 * np.sqrt(E / P / 2))
b = np.full(N+1, xi ** 0.9 * np.sqrt(E / P / 2))
# b -= b
# a[0] = np.sqrt(E / P) / 2
# b[0] = 0
