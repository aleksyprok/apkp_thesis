...
\end{equation}
We set $v_{A-} = v_{A+} / 21$ and $\omega_r = \omega_{11}$.
The boundary conditions in $x$ are
\begin{gather}
    \label{eq:ux_x_min_bc_oblique}
    u_x'(x_{min},z) = -u_0x_{i,21}\ln(X_{21})\nabla_\perp'\phi_{21}(z), \\
    \label{eq:b_par_x_min_bc_oblique}
    \nabla_\perp' \hat{b}_{||}'(x_{min},z)=2iu_0\frac{\omega x_{i,21}}{a_0v_A^2(0,z)}\phi_{21}(z),
\end{gather}
where
\begin{equation}
    X_{21} = \frac{x_{i,21}}{x-ix_{i,21}},
\end{equation}
